    ...
    Row(
        ...
        when(blurUiState){
            is BlurUiState.Default -> {
                Button(onClick = onStartClick) { Text(text = stringResource(id = R.string.start))}
            }
            is BlurUiState.Loading -> {
                FilledTonalButton(onClick = onCancelClick) { Text(text = stringResource(id = R.string.cancel_work))}
